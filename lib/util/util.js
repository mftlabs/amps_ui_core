...
export const useRequest = ({ useAuthContext }) => {
  const { renew } = useRenew({ useAuthContext });
...
export const useRenew = ({ useAuthContext }) => {
...
export const useSocket = (channel_name, options = {}, useAuthContext) => {
  ...
  function joinChannel() {
    ...
    cnl
      .join()
      .receive("ok", (resp) => {
        options.subscriptions.forEach(({ event, callback }) => {
          cnl.on(event, (msg) => {
            callback(msg);
          });
        });
